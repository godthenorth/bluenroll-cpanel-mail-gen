...
def send_start_task_email():
    # Set up the SMTP server
    s = smtplib.SMTP(host='mail.bluenroll.co.za', port=587)
    s.starttls()
    s.login('info@bluenroll.co.za', 'mmDev2024%%')

    # Create the email
    msg = MIMEMultipart()
    msg['From'] = 'info@bluenroll.co.za'
    msg['To'] = 'godthenorth@gmail.com'
    msg['Subject'] = 'Task Initialised, Database Is Being Balanced'

    # The email body
    body = f'Dear BluEnSKG Admin (MM.Dev),\n\nThe inversion task has been successfully initialised and the database is being balanced. {len(emails)} emails have been appended.\n\nBest Regards,\nMyself I Guess'
...
def send_end_task_email():
    ...
    msg['From'] = 'info@bluenroll.co.za'
    msg['To'] = 'godthenorth@gmail.com'
    msg['Subject'] = 'Task Done, Database Balanced'

    # The email body
    body = f'Dear BluEnSKG Admin (MM.Dev),\n\nThe inversion task has been successfully completed and the database is balanced. {len(emails)} emails have been appended.\n\nBest Regards,\nMyself I Guess'
    msg.attach(MIMEText(body, 'plain'))
...
def second_mail_creation_procedure():
    ...
    cursor = db.cursor()
    #Fetch the database from down to up
    cursor.execute("SELECT id, name, surname FROM applicants WHERE second_mail IS NULL ORDER BY id DESC")
